...
const joinForm: HTMLFormElement | null = document.querySelector("#join");
if (!joinForm) throw new Error("missing #join form");

const chatForm: HTMLFormElement | null = document.querySelector("#chatbox");
...
const presenter = new Presenter(domain, {
    joinForm,
    roomEl,
    createMemberEl({username, avatar}: Pick<MemberInfo, "username" | "avatar">, isMe?: boolean) {
